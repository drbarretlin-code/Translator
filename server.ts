...
async function startServer() {
  ...
  // 翻譯 API (使用 Server-Sent Events 串流回應)
  app.post("/api/translate", async (req, res) => {
    const { text, sourceLang, targetLang, id, history } = req.body;
...
Constraints_and_Edge_Cases:
  ...
  - 若輸入句子因省略主詞或上下文而產生嚴重歧義，請預設採用最常見的通用/商務情境進行翻譯。`;
      ...
      for await (const chunk of responseStream) {
        ...
        res.write(`data: ${JSON.stringify({ type: 'stream', id, translated: fullTranslation })}\n\n`);
      }
      
      // 將最終翻譯結果推播回前端
      res.write(`data: ${JSON.stringify({ type: 'response', id, original: text, translated: fullTranslation, status: 'success' })}\n\n`);
      res.end();
    } catch (error: any) {
      ...
      // 發生錯誤時回傳錯誤訊息給前端
      res.write(`data: ${JSON.stringify({ type: 'error', id, error: errorMessage, status: 'error' })}\n\n`);
      res.end();
    }
  });
